...
/*******************************************************************************
* Function Name  : LCD_DrawOLEDPageBitmapCentered
* Description    : Draws a 1-bit OLED page-format bitmap centered on LCD.
* Input          : - bitmap: page-format bitmap, 8 vertical pixels per byte.
*                  - Width: bitmap width in pixels.
*                  - Height: bitmap height in pixels.
* Output         : None
* Return         : None
*******************************************************************************/
void LCD_DrawOLEDPageBitmapCentered(const u8 *bitmap, u16 Width, u16 Height)
{
    u16 savedGpioC;
    u16 drawWidth;
    u16 drawHeight;
    ...
    savedGpioC = GPIOC->ODR;

    for(row = 0; row < drawHeight; row++)
    {
        srcY = srcYOffset + row;
        LCD_SetCursor((u8)(lcdRowOffset + row), lcdColRight);
        LCD_WriteRAM_Prepare();

        for(srcX = srcXOffset + drawWidth; srcX > srcXOffset; srcX--)
        {
            pixelX = srcX - 1;
            data = bitmap[(srcY / 8) * Width + pixelX];
